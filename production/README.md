...
The Typst source also appends an appendix section, `Appendix A. Detailed Statistical Analysis Output`, generated from `analysis_pipeline/output/survey_data_results_comprehensive.md`. The appendix is added only to the editable Typst pipeline (`typst_content/`) and does not affect Method A or Method B outputs. Numerical content is preserved exactly; only the visual formatting is upgraded via Typst tables, an italic subtitle, and structured bullet lists.
