...
If optional tools such as Typst or Pandoc are missing, the build prints a warning and continues with the available outputs. Typst is needed for PDF compilation in the Typst paths; the dedicated `typst_content` DOCX writer does not require Pandoc.
...
typst_content/output/survey_results.pdf
typst_content/output/survey_results.docx
```
...
The `typst_content/` directory contains a single editable Typst file, `research.typ`, generated from the current manuscript content. It includes a formal cover page with an optional university logo loaded from `figures/`, supervisor certification, dedication, acknowledgment, automatic table of contents, lists of figures/tables/abbreviations, page borders, Roman-numbered preliminary pages, Arabic numbering from the abstract onward, and chapter running heads after chapter-opening pages. The source links to the existing `figures/` assets and, when Typst is available, compiles `typst_content/output/research.pdf`. The matching `typst_content/output/research.docx` is generated by Python from the same structured Typst-content model rather than by Pandoc, because direct Typst-to-DOCX conversion loses custom body content in this project. The same build step also compiles the standalone survey results PDF from `typst_content/survey_results.typ` when Typst is available and always writes an editable `typst_content/output/survey_results.docx` from `survey_data_results.md`, reshaping the raw result tables into the same section order, captions, page breaks, full-page-width tables, and bordered navy/gold presentation style used by `survey_results.pdf`. An emergency Method A DOCX fallback remains only if the dedicated Typst-content DOCX writer fails.
