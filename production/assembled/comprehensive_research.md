...
This section presents the survey results from 877 respondents in their original tabular form, covering sample composition, the four core attitude outcomes, the three core belief items (Q11, Q12, Q13), bivariate correlations among primary variables, recommendation willingness stratified by prior medication use (Q31), the hierarchical and multinomial logistic regressions of recommendation willingness (Q8), the contact-hypothesis comparison between users and non-users, and the exploratory k-means stigma phenotypes. Numerical interpretation, practical-magnitude judgments, and reporting hierarchy follow in IV.B through IV.H, where the same values are read in narrative form against the analysis plan defined in Methodology.

The achieved sample of 877 respondents had the following demographic composition, computed as percentage of valid responses for each item.
...
The four general attitude items (Q6 safety perception, Q7 acceptability, Q8 recommendation willingness, Q9 social concern) were measured with a three-category response set, and their distributions across the full sample are shown below.
...
| Social concern (Q9) | 37.77% | 13.09% | 49.14% |

The three core belief items (Q11 doctors prescribe psychiatric medications more than necessary, Q12 most psychiatric medications cause psychological or physical dependence, Q13 modern psychiatric medications are safer than older ones) were measured on a five-point Likert scale, and their distributions are summarized in collapsed agreement form below.
...
The bivariate correlation matrix among the primary belief items (Q11–Q13) and the three-category attitude outcomes (social concern Q9, acceptability Q7, recommendation willingness Q8) is shown below.
...
Recommendation willingness (Q8) was also broken down by prior psychiatric medication use status (Q31).
...
The hierarchical block logistic regression used a binary recommendation outcome (Q8 yes vs no) and added predictor blocks sequentially. Complete-case sample size for the hierarchical model was n=647. Block-level model fit is summarized below.

| Model block | Predictors | McFadden pseudo R² | LLR p-value |
|---|---|---:|---:|
| Block 1 (mle) | Age, Gender, Education, Marital status | 0.0040 | 0.5037 |
| Block 2 (mle) | Block 1 + Prior use | 0.0116 | 0.0847 |
| Block 3 (mle) | Block 2 + Q11, Q12, Q13, Fear | 0.0686 | <0.0001 |

The adjusted odds ratios from the final hierarchical block (Block 3) are shown below.

| Predictor | Adjusted OR (95% CI) | p-value |
|---|---:|---:|
| Intercept | 1.128 (0.298 to 4.273) | 0.8597 |
| Age (binary) | 0.961 (0.583 to 1.582) | 0.8752 |
| Gender (binary) | 1.516 (1.043 to 2.205) | 0.0294 |
| Education (binary) | 1.381 (0.750 to 2.542) | 0.3005 |
| Marital status (binary) | 1.075 (0.632 to 1.827) | 0.7904 |
| Prior use (binary) | 1.342 (0.800 to 2.252) | 0.2645 |
| Q11 | 0.830 (0.696 to 0.991) | 0.0396 |
| Q12 | 0.869 (0.701 to 1.078) | 0.2008 |
| Q13 | 1.507 (1.248 to 1.820) | <0.0001 |
| Fear (binary) | 0.504 (0.358 to 0.711) | <0.0001 |

A separate sensitivity model that added Q6 (safety perception) and Q7 (acceptability) used complete-case n=406 and produced McFadden pseudo R² of 0.2440 (fit type: mle). This model is reported separately because Q6 and Q7 are conceptually proximate to the recommendation outcome.

The multinomial logistic regression preserved the three-category recommendation structure (Q8 No / Yes / Not sure) with complete-case n=837 and model log-likelihood -748.910 (fit type: mle). The reference category is Q8=No, and the table below shows the relative risk ratios for the two non-reference outcome equations.

| Outcome equation | Predictor | RRR (95% CI) | p-value |
|---|---|---:|---:|
| Q8=Yes vs ref | Intercept | 0.936 (0.264 to 3.317) | 0.9189 |
| Q8=Yes vs ref | Age (binary) | 0.947 (0.599 to 1.496) | 0.8139 |
| Q8=Yes vs ref | Gender (binary) | 1.558 (1.097 to 2.214) | 0.0133 |
| Q8=Yes vs ref | Education (binary) | 1.177 (0.661 to 2.095) | 0.5797 |
| Q8=Yes vs ref | Marital status (binary) | 0.967 (0.598 to 1.563) | 0.8903 |
| Q8=Yes vs ref | Prior use (binary) | 1.387 (0.863 to 2.229) | 0.1763 |
| Q8=Yes vs ref | Q11 | 0.873 (0.742 to 1.028) | 0.1028 |
| Q8=Yes vs ref | Q12 | 0.794 (0.651 to 0.970) | 0.0239 |
| Q8=Yes vs ref | Q13 | 1.585 (1.328 to 1.892) | <0.0001 |
| Q8=Not sure vs ref | Intercept | 0.304 (0.046 to 2.013) | 0.2170 |
| Q8=Not sure vs ref | Age (binary) | 1.094 (0.549 to 2.180) | 0.7983 |
| Q8=Not sure vs ref | Gender (binary) | 2.171 (1.211 to 3.894) | 0.0093 |
| Q8=Not sure vs ref | Education (binary) | 1.300 (0.526 to 3.211) | 0.5692 |
| Q8=Not sure vs ref | Marital status (binary) | 1.713 (0.863 to 3.399) | 0.1237 |
| Q8=Not sure vs ref | Prior use (binary) | 1.401 (0.704 to 2.789) | 0.3373 |
| Q8=Not sure vs ref | Q11 | 0.846 (0.662 to 1.080) | 0.1799 |
| Q8=Not sure vs ref | Q12 | 0.890 (0.660 to 1.199) | 0.4438 |
| Q8=Not sure vs ref | Q13 | 1.064 (0.820 to 1.381) | 0.6404 |

The contact-hypothesis comparison used Q31 prior psychiatric medication use to split respondents into Users (n=127) and Non-Users (n=716), and tested the three core belief items with both rank-based (Mann-Whitney U with Cliff's delta) and nominal (chi-square with Cramer's V) methods.

| Item | User median | Non-user median | M-W U p-value | Cliff's delta | Chi-square p-value | Cramer's V | N used |
|---|---:|---:|---:|---:|---:|---:|---:|
...
The exploratory k-means stigma phenotype analysis was applied to standardized Q11–Q13 with k=2, k=3, and k=4. Silhouette scores guided cluster selection.
...
The four-cluster solution (k=4, selected by maximum silhouette) produced the following profile mean structure on the original Likert metric.
...
These k-means profiles are data-driven exploratory groupings and not confirmed latent classes; the silhouette values express relative fit among the tested k values in this dataset only.
