...
ADVANCED_MD = OUTPUT_DIR / "survey_data_results_advanced.md"
...
def main():
    ...
    lines = []
    lines.append(f"# Advanced Statistical Extension — Psychiatric Medication Use and Public Acceptance in Iraq (N={n_total})")
    lines.append("")
    lines.append("This report appends advanced analyses to the baseline script without replacing existing baseline outputs.")
    lines.append("")
    ...
    ADVANCED_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Advanced analysis written to: {ADVANCED_MD}")
    if RADAR_PNG.exists():
